...
ggplot(freq_df_long_d, aes(x = reorder(country, percent), y = percent, fill = answer)) +
  geom_bar(stat = "identity") +  # Use identity to show actual values
...
freq_df_summaryh <- freq_df_summaryh %>%
  mutate(
    total_answers = `Very helpful` + `Somewhat helpful` + `Not helpful`,  # Sum up all responses
...
freq_df_longh <- freq_df_summaryh %>%
  ...
  mutate(percent = ifelse(answer2 == "percent_not_helpful", -percent, percent))  # Make No percentage negative
...
ggplot(freq_df_longh, aes(x = reorder(country, percent), y = percent, fill = answer2)) +
  ...
  scale_fill_manual(values = c(
    ...
    "percent_not_helpful" = "red")) +  # Assign colors to each level
...
ggplot(final, aes(x = OBS_VALUE, y = percent_yes_m)) +
  geom_point(aes(color = country), size = 3) +  # Scatter plot by country
  geom_smooth(method = "lm", color = "red", se = FALSE, size = 1) +  # Correlation line
  labs(
...
ggplot(country_yes_data, aes(x = reorder(country, yes_percentage), y = yes_percentage, fill = variable)) +
  ...
  labs(title = "Percentage of 'Yes' Answers by Country for Meds and Depr_more", 
       x = "Country", 
       y = "Percentage of 'Yes' Answers") +
  scale_fill_manual(values = c("skyblue", "lightgreen")) +  # Custom bar colors
  theme_minimal()
...
ggplot(central_asia_graph, aes(x = reorder(country, percent_yes), y = percent_yes, fill = variable)) +
  ...
  scale_fill_manual(values = c("skyblue", "lightgreen", "lightyellow", "aquamarine", "pink")) +  # Custom bar colors
  theme_minimal()
